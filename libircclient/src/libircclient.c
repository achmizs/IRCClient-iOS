...
static void libirc_process_incoming_data (irc_session_t * session, size_t process_length)
{
	...
	else
	{
		...
		else if (!strncmp (command, "PRIVMSG", buf_end - command))
		{
			...
			{ 
				...
				if (params[1][0] == 0x01 && params[1][msglen-1] == 0x01)
				{
					...
					else if (!strncasecmp(ctcp_buf, "ACTION ", 7) && session->callbacks.event_ctcp_action)
					{
				...
				/*
				 * Check to see if first parameter is a channel name
				 */
				else if (params[0][0] == '#' || params[0][0] == '&' || params[0][0] == '!' || params[0][0] == '+')
				{
				...
				{
					if (session->callbacks.event_server_msg)
						(*session->callbacks.event_server_msg) (session, command, prefix, params, paramindex);
				}
			}
		}
		else if (!strncmp (command, "NOTICE", buf_end - command))
		{
			...
			{
				if (session->callbacks.event_server_notice)
					(*session->callbacks.event_server_notice) (session, command, prefix, params, paramindex);
			}
		}
